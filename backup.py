...
def calculate_flange_price(psi, suction_flange_size, discharge_flange_size, liquid_end_material):
    ...
    if suction_price is None or discharge_price is None:
        return {"error": "Invalid flange size or material"}
...
def find_best_pump(gph=None, lph=None, psi=None, bar=None, hz=None, simplex_duplex=None, want_motor=None, motor_type=None, motor_power=None, spm=None, diaphragm=None, liquid_end_material=None, leak_detection=None, phase=None, degassing=None, flange=None, balls_type=None, suction_lift=None, ball_size=None, suction_flange_size=None, discharge_flange_size=None):
    ...
    for pump in pumps:
        ...
        # Add flange price only if flange is "Yes"
        if flange and flange.lower() == "yes":
            ...
            if "error" in flange_price_result:
                return flange_price_result

            total_flange_price = flange_price_result["total_flange_price"]
            # Add the flange price to the total price
            if isinstance(total_price, str):
                total_price = f"{total_price} + ${total_flange_price}"
            else:
                total_price += total_flange_price
...
def get_pump():
    try:
        ...
        print(f"Received parameters: gph={gph}, psi={psi}, hz={hz}, simplex_duplex={simplex_duplex}, want_motor={want_motor}, motor_type={motor_type}, motor_power={motor_power}, spm={spm}, diaphragm={diaphragm}, liquid_end_material={liquid_end_material}, leak_detection={leak_detection}, phase={phase}, degassing={degassing}, flange={flange}, balls_type={balls_type}, suction_lift={suction_lift}, ball_size={ball_size}, suction_flange_size={suction_flange_size}, discharge_flange_size={discharge_flange_size}")
